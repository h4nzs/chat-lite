...
interface AuthFormProps {
  onSubmit: (v: { a: string; b?: string; d?: string; name?: string }) => Promise<void>;
  button: string;
  isRegister?: boolean;
  disabled?: boolean;
}

export default function AuthForm({ onSubmit, button, isRegister = false, disabled = false }: AuthFormProps) {
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        ...
        setIsLoading(true)
        try {
          await onSubmit({ a: emailOrUsername, b: password, d: username, name })
        }
        catch (ex: unknown) {
