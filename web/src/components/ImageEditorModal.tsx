    ...
    <div className="fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center overscroll-none">
      ...
      <div className="flex items-center justify-between p-4 bg-gradient-to-b from-black/80 to-transparent z-20 absolute top-0 left-0 right-0 pointer-events-auto pt-safe">
        <button 
          ...
          className="p-3 bg-black/10 dark:bg-white/10 hover:bg-red-500/20 text-white rounded-2xl backdrop-blur-xl transition-all active:scale-95"
        >
          <FiX size={22} />
        </button>
        
        <div className="flex gap-2 p-1 bg-black/5 dark:bg-white/5 rounded-2xl backdrop-blur-md border border-text-secondary/10">
          <button 
            onClick={() => canvasRef.current?.undo()} 
            aria-label={t('modals:editor.undo', 'Undo')}
            title={t('modals:editor.undo', 'Undo')}
            className="p-3 text-white/80 hover:text-white hover:bg-black/10 dark:hover:bg-black/10 dark:bg-white/10 rounded-xl transition-all"
          >
      ...
      <div className="relative w-full h-full flex items-center justify-center p-4 md:p-12 lg:p-20 overflow-hidden">
          <div 
            ...
            className="shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-text-secondary/10 overflow-hidden touch-none cursor-crosshair bg-black relative"
          >
             <ReactSketchCanvas 
                ref={canvasRef} 
                strokeWidth={5} 
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col md:flex-row items-center justify-between gap-6 z-20 pb-safe">
        {/* Color Palette */}
        <div className="flex gap-3 bg-black/40 p-2.5 rounded-full backdrop-blur-2xl border border-text-secondary/10 shadow-2xl">
          {COLORS.map(c => (
            <button 
              key={c} 
              onClick={() => setColor(c)} 
